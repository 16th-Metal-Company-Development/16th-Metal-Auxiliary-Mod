class XtdGearInfos {
    class CfgWeapons {
        // [16th] Trooper Helmet variants (Phase 2)
        class helmet_16th_trp          { model = "16th_Trooper_Helmet"; camo = "Trooper"; };
        class helmet_16th_cadet        { model = "16th_Trooper_Helmet"; camo = "Cadet"; };
        class helmet_16th_vttrp        { model = "16th_Trooper_Helmet"; camo = "Veteran"; };
        class helmet_16th_special      { model = "16th_Trooper_Helmet"; camo = "Specialist"; };
        class helmet_16th_nco          { model = "16th_Trooper_Helmet"; camo = "NCO"; };
        class helmet_16th_snco         { model = "16th_Trooper_Helmet"; camo = "SNCO"; };
        class helmet_16th_officer      { model = "16th_Trooper_Helmet"; camo = "Officer"; };
        class helmet_16th_medic        { model = "16th_Trooper_Helmet"; camo = "Medic"; };
        class helmet_16th_eod          { model = "16th_Trooper_Helmet"; camo = "EOD"; };
        class helmet_16th_hazmat       { model = "16th_Trooper_Helmet"; camo = "Hazmat"; };
        class helmet_16th_chetiah      { model = "16th_Trooper_Helmet"; camo = "Chetiah"; };
        class helmet_16th_arch         { model = "16th_Trooper_Helmet"; camo = "Arch"; };
        class helmet_16th_krieg        { model = "16th_Trooper_Helmet"; camo = "Krieg"; };
        class helmet_16th_blackrose    { model = "16th_Trooper_Helmet"; camo = "Blackrose"; };
        class helmet_16th_merlin       { model = "16th_Trooper_Helmet"; camo = "Merlin"; };
        class helmet_16th_moldy        { model = "16th_Trooper_Helmet"; camo = "Moldy"; };
        class helmet_16th_william      { model = "16th_Trooper_Helmet"; camo = "William"; };
        class helmet_16th_wolf         { model = "16th_Trooper_Helmet"; camo = "Wolf"; };
        class helmet_16th_blatt        { model = "16th_Trooper_Helmet"; camo = "Blatt"; };
        class helmet_16th_butcher      { model = "16th_Trooper_Helmet"; camo = "Butcher"; };
        class helmet_16th_winters      { model = "16th_Trooper_Helmet"; camo = "Winters"; };
        class helmet_16th_roz          { model = "16th_Trooper_Helmet"; camo = "Roz"; };
        class helmet_16th_six          { model = "16th_Trooper_Helmet"; camo = "Six"; };
        class helmet_16th_gambit       { model = "16th_Trooper_Helmet"; camo = "Gambit"; };
        class helmet_16th_frost        { model = "16th_Trooper_Helmet"; camo = "Frost"; };
        class helmet_16th_smith        { model = "16th_Trooper_Helmet"; camo = "Smith"; };
        class helmet_16th_fish         { model = "16th_Trooper_Helmet"; camo = "Fish"; };
        class helmet_16th_mace         { model = "16th_Trooper_Helmet"; camo = "Mace"; };
        class helmet_16th_kick         { model = "16th_Trooper_Helmet"; camo = "Kick"; };
        class helmet_16th_nix          { model = "16th_Trooper_Helmet"; camo = "Nix"; };
        class helmet_16th_maple        { model = "16th_Trooper_Helmet"; camo = "Maple"; };
        class helmet_16th_hood         { model = "16th_Trooper_Helmet"; camo = "Hood"; };
        class helmet_16th_sam          { model = "16th_Trooper_Helmet"; camo = "Sam"; };
        class helmet_16th_ben          { model = "16th_Trooper_Helmet"; camo = "Ben"; };
        class helmet_16th_creed        { model = "16th_Trooper_Helmet"; camo = "Creed"; };
        class helmet_16th_karma        { model = "16th_Trooper_Helmet"; camo = "Karma"; };
        class helmet_16th_prez         { model = "16th_Trooper_Helmet"; camo = "Prez"; };
        class helmet_16th_cougar       { model = "16th_Trooper_Helmet"; camo = "Cougar"; };
        class helmet_16th_blowfish     { model = "16th_Trooper_Helmet"; camo = "Blowfish"; };
        class helmet_16th_spice        { model = "16th_Trooper_Helmet"; camo = "Spice"; };
        class helmet_16th_talk         { model = "16th_Trooper_Helmet"; camo = "Talk"; };
        class helmet_16th_avalanche    { model = "16th_Trooper_Helmet"; camo = "Avalanche"; };
        class helmet_16th_maxson       { model = "16th_Trooper_Helmet"; camo = "Maxson"; };
        class helmet_16th_ronin_barc   { model = "16th_Trooper_Helmet"; camo = "Ronin"; };

        // [16th] Airborne Trooper Helmet variants
        class helmet_16th_ab_trp       { model = "16th_Airborne_Helmet"; camo = "Trooper"; };
        class helmet_16th_ab_vtrp      { model = "16th_Airborne_Helmet"; camo = "Veteran"; };
        class helmet_16th_ab_cpl       { model = "16th_Airborne_Helmet"; camo = "Corporal"; };
        class helmet_16th_ab_sgt       { model = "16th_Airborne_Helmet"; camo = "Sergeant"; };
        class helmet_16th_ab_ssgt      { model = "16th_Airborne_Helmet"; camo = "StaffSergeant"; };
        class helmet_16th_ab_cfs       { model = "16th_Airborne_Helmet"; camo = "FirstSergeant"; };
        class helmet_16th_ab_sgm       { model = "16th_Airborne_Helmet"; camo = "SergeantMajor"; };
        class helmet_16th_ab_medic     { model = "16th_Airborne_Helmet"; camo = "Medic"; };
        class helmet_16th_ab_blackrose { model = "16th_Airborne_Helmet"; camo = "Blackrose"; };
        class helmet_16th_ab_kick      { model = "16th_Airborne_Helmet"; camo = "Kick"; };

        // [16th] Goff Helmet (ARF) variants
        class helmet_16th_arf          { model = "16th_Goff_Helmet"; camo = "Trooper"; };
        class helmet_16th_arf_nco      { model = "16th_Goff_Helmet"; camo = "NCO"; };
        class helmet_16th_kix          { model = "16th_Goff_Helmet"; camo = "Kix"; };
        class helmet_16th_convor       { model = "16th_Goff_Helmet"; camo = "Convor"; };
        class helmet_16th_maple_arf    { model = "16th_Goff_Helmet"; camo = "Maple"; };
        class helmet_16th_blue_arf     { model = "16th_Goff_Helmet"; camo = "Blue"; };

        // [16th] Cold Assault Helmet variants
        class helmet_16th_sm_trp        { model = "16th_CA_Helmet"; camo = "Trooper"; };
        class helmet_16_sm_nco          { model = "16th_CA_Helmet"; camo = "NCO"; };
        class helmet_16_sm_off          { model = "16th_CA_Helmet"; camo = "Officer"; };
        class helmet_16_sm_rto          { model = "16th_CA_Helmet"; camo = "RTO"; };

        // [16th] Thunder Helmet (Pilot) variants
        class helmet_16th_plt           { model = "16th_Thunder_Helmet"; camo = "Pilot"; };
        class helmet_16th_plt_fool      { model = "16th_Thunder_Helmet"; camo = "Fool"; };
        class helmet_16th_plt_sunto     { model = "16th_Thunder_Helmet"; camo = "Sunto"; };

        // [16th] Trooper Armor variants (Phase 2)
        class armor_16th_trp            { model = "16th_Trooper_Armor"; camo = "Trooper"; };
        class armor_16th_cadet          { model = "16th_Trooper_Armor"; camo = "Cadet"; };
        class armor_16th_arty           { model = "16th_Trooper_Armor"; camo = "Dragon"; };
        class armor_16th_arty_nco       { model = "16th_Trooper_Armor"; camo = "DragonNCO"; };
        class armor_16th_mechanized     { model = "16th_Trooper_Armor"; camo = "Reaper"; };
        class armor_16th_mechanized_nco { model = "16th_Trooper_Armor"; camo = "ReaperNCO"; };
        class armor_16th_vttrp          { model = "16th_Trooper_Armor"; camo = "Veteran"; };
        class armor_16th_special        { model = "16th_Trooper_Armor"; camo = "Specialist"; };
        class armor_16th_nco            { model = "16th_Trooper_Armor"; camo = "NCO"; };
        class armor_16th_snco           { model = "16th_Trooper_Armor"; camo = "SNCO"; };
        class armor_16th_officer        { model = "16th_Trooper_Armor"; camo = "Officer"; };
        class armor_16th_med            { model = "16th_Trooper_Armor"; camo = "Medic"; };
        class armor_16th_eod            { model = "16th_Trooper_Armor"; camo = "EOD"; };
        class armor_16th_ronin          { model = "16th_Trooper_Armor"; camo = "Ronin"; };
        class armor_16th_moldy_zeus     { model = "16th_Trooper_Armor"; camo = "MoldyZeus"; };
        class armor_16th_moldy          { model = "16th_Trooper_Armor"; camo = "Moldy"; };
        class armor_16th_blatt          { model = "16th_Trooper_Armor"; camo = "Blatt"; };
        class armor_16th_blackrose      { model = "16th_Trooper_Armor"; camo = "Blackrose"; };
        class armor_16th_prez           { model = "16th_Trooper_Armor"; camo = "Prez"; };
        class armor_16th_hood           { model = "16th_Trooper_Armor"; camo = "Hood"; };
        class armor_16th_kix            { model = "16th_Trooper_Armor"; camo = "Kix"; };
        class armor_16th_winters        { model = "16th_Trooper_Armor"; camo = "Winters"; };
        class armor_16th_six            { model = "16th_Trooper_Armor"; camo = "Six"; };
        class armor_16th_cooger         { model = "16th_Trooper_Armor"; camo = "Cougar"; };
        class armor_16th_maxson         { model = "16th_Trooper_Armor"; camo = "Maxson"; };
        class armor_16th_merlin         { model = "16th_Trooper_Armor"; camo = "Merlin"; };
        class armor_16th_avalanche      { model = "16th_Trooper_Armor"; camo = "Avalanche"; };

        // [16th] Airborne Trooper Armor variants
        class armor_16th_ab_trp        { model = "16th_Airborne_Armor"; camo = "Trooper"; };
        class armor_16th_ab_vtrp       { model = "16th_Airborne_Armor"; camo = "Veteran"; };
        class armor_16th_ab_cpl        { model = "16th_Airborne_Armor"; camo = "Corporal"; };
        class armor_16th_ab_sgt        { model = "16th_Airborne_Armor"; camo = "Sergeant"; };
        class armor_16th_ab_ssgt       { model = "16th_Airborne_Armor"; camo = "StaffSergeant"; };
        class armor_16th_ab_cfs        { model = "16th_Airborne_Armor"; camo = "FirstSergeant"; };
        class armor_16th_ab_sgm        { model = "16th_Airborne_Armor"; camo = "SergeantMajor"; };
        class armor_16th_ab_medic      { model = "16th_Airborne_Armor"; camo = "Medic"; };

        // [16th] Goff Armor (ARF) variants
        class armor_16th_arf           { model = "16th_Goff_Armor"; camo = "Trooper"; };
        class armor_16th_arf_nco       { model = "16th_Goff_Armor"; camo = "NCO"; };

        // [16th] Cold Assault Armor variants
        class armor_16_ca_trp          { model = "16th_CA_Armor"; camo = "Trooper"; };
        class armor_16_ca_nco          { model = "16th_CA_Armor"; camo = "NCO"; };
        class armor_16_ca_off          { model = "16th_CA_Armor"; camo = "Officer"; };
        class armor_16_ca_med          { model = "16th_CA_Armor"; camo = "Medic"; };
        class armor_16_ca_eod          { model = "16th_CA_Armor"; camo = "EOD"; };
    };
};